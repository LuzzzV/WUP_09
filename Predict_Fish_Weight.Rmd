---
title: "Weight of Fish"
author: "Luz Vargas"
date: "`r Sys.Date()`"  
output: 
  html_document:
    number_sections: true
    toc: true
    toc_depth: 5

 
    
---


```{r include=FALSE,echo=FALSE}
require(tigerstats)
require(tidyverse)
library(leaps)
```


## import fish CSV using the import function that Luz found

# Predicting Fish Weight


## Perch
```{r}
Fish = read.csv('Fish.csv')
Perch = filter (Fish,Species == 'Perch')
Perch = subset(Perch, select = -Species)
weightPerch = lm(Weight~Length1 + Length2 +Length3 + Height + Width, data = Perch)  
summary.lm(weightPerch)  
```

### Best Models


### Best Two regressor model



```{r}
regsubsets.out <-
    regsubsets(Weight ~. , data = Perch,
               nbest = 1,       # 1 best model for each number of predictors
               nvmax = NULL,    # NULL for no limit on number of variables
               force.in = NULL, force.out = NULL,
               method = "exhaustive")

summary.out <- summary(regsubsets.out)
as.data.frame(summary.out$outmat)
```

The best two variables are Height and Width.


## Pike


```{r}

Pike = filter (Fish,Species == 'Pike')
Pike = subset(Pike, select = -Species)
weightPike = lm(Weight~Length1 + Length2 +Length3 + Height + Width, data = Pike)  
summary.lm(weightPike)  
```



### Best Models

### Best Two regressor model


```{r}
regsubsets.out <-
    regsubsets(Weight ~. , data = Pike,
               nbest = 1,       # 1 best model for each number of predictors
               nvmax = NULL,    # NULL for no limit on number of variables
               force.in = NULL, force.out = NULL,
               method = "exhaustive")

summary.out <- summary(regsubsets.out)
as.data.frame(summary.out$outmat)
```

The best model using two predictors for Pike are length 3 and length 1.

## Roach


```{r}

Roach = filter (Fish,Species == 'Roach')
Roach = subset(Roach, select = -Species)
weightRoach = lm(Weight~Length1 + Length2 +Length3 + Height + Width, data = Roach)  
summary.lm(weightRoach)  
```


### Best Models

### Best Two regressor model


```{r}
regsubsets.out <-
    regsubsets(Weight ~. , data = Roach,
               nbest = 1,       # 1 best model for each number of predictors
               nvmax = NULL,    # NULL for no limit on number of variables
               force.in = NULL, force.out = NULL,
               method = "exhaustive")

summary.out <- summary(regsubsets.out)
as.data.frame(summary.out$outmat)
```

The best models using two predictors for Roach are Width and Length 3.

## Final Summary Statements

The adjusted R-square for Perch is 0.9372, The adjusted R-square for Pike is 0.9794, and the adjusted R-square for Roach is 0.821.